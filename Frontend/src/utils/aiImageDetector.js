...
export async function classifyCivicContent(predictions) {
  ...
  let civicType = 'other';
  if (isCivic) {
    const potholeKeywords = ['pothole', 'hole', 'asphalt', 'crater', 'pit', 'manhole', 'crack'];
    const garbageKeywords = ['trash', 'garbage', 'dump', 'waste', 'plastic', 'debris', 'rubble'];
    const waterKeywords = ['leak', 'water', 'flood', 'drain', 'pipe', 'puddle', 'stagnant', 'sewer'];
    const lightKeywords = ['light', 'lamp', 'streetlight', 'pole'];
    const roadKeywords = ['road', 'street', 'concrete', 'sidewalk', 'pavement', 'curb', 'barrier'];
